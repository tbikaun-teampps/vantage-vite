...
import { AssessmentsService, CreateAssessmentData } from "../../services/AssessmentsService.js";
...
export async function assessmentsRouter(fastify: FastifyInstance) {
  ...
  fastify.get("/:assessmentId/interviews", async (request, reply) => {
    ...
    // Implement logic to fetch interviews related to a specific assessment by ID
    return reply.send({
      message: `Fetched interviews for assessment ${assessmentId}`,
    });
  });
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        const duplicatedAssessment = await assessmentService.duplicateAssessment(
          Number(assessmentId)
        );
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Failed to duplicate assessment";
