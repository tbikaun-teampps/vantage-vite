...
export class HeatmapService {
      ...
      interview.interview_responses?.forEach((response) => {
        response.interview_response_roles?.forEach((roleData) => {
          ...
          flattened.push({
            ...
            question_id: questionDetails?.question_id || response.questionnaire_question_id,
  ...
  private generateHeatmap(flatData: any[]) {
    ...
    const xLabels = [...new Set(metricResults.average_score.map((d) => d.x))].sort();
    const yLabels = [...new Set(metricResults.average_score.map((d) => d.y))].sort();
