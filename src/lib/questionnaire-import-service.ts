...
export class QuestionnaireImportService {
          ...
          steps: (section.steps || [])
            ...
            .map((step: any, stepIndex: number) => ({
              ...
              questions: (step.questions || [])
                .filter((question: any) => question.title && question.title.trim()) // Filter out empty questions
   ...
   */
  private static validateRatingScales(ratingScales: any[], errors: ValidationError[]) {
   ...
   */
  private static validateSteps(steps: any[], errors: ValidationError[], sectionPath: string) {
   ...
   */
  private static validateQuestions(questions: any[], errors: ValidationError[], stepPath: string) {
  ...
  private static calculateStats(data: any) {
    ...
    const sectionsCount = data.sections?.filter((s: any) => s.title?.trim()).length || 0;
    ...
    data.sections?.forEach((section: any) => {
      if (section.steps && Array.isArray(section.steps)) {
        ...
        validSteps.forEach((step: any) => {
          if (step.questions && Array.isArray(step.questions)) {
            questionsCount += step.questions.filter((q: any) => q.title?.trim()).length;
          }
        });
      }
    });
  ...
  static detectRatingScaleConflicts(
    ...
    importedScales.forEach(importedScale => {
      const nameConflict = existingScales.find(
        existing => existing.name.toLowerCase() === importedScale.name.toLowerCase()
      );
              ...
              questions: [
                {
                  ...
                  question_text: "How effectively does the organization communicate its vision and mission?",
                  context: "Evaluate the clarity and communication of organizational direction",
                  order: 1
                },
                {
                  title: "Goal Setting",
                  question_text: "How well does the organization set and track strategic goals?",
                  context: "Assess the goal-setting process and monitoring mechanisms",
                  order: 2
                }
              ]
            }
              ...
              questions: [
                {
                  ...
                  question_text: "How well are standard operating procedures documented and followed?",
  ...
  static exportQuestionnaire(questionnaire: any): string {
    const exportData: ImportQuestionnaire = {
      ...
      rating_scales: questionnaire.rating_scales?.map((scale: any) => ({
        name: scale.name,
        description: scale.description || '',
        value: scale.value
      })) || [],
